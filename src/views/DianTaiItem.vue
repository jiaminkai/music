    ...
    <div class="mainc">
      <div class="userbox">
        ...
        <div class="userdesc">
          ...
          <div class="cary">
            <span class="iconfont">&#xe605;</span><span>{{this.data.description}}</span>
            <span class="el-icon-star-off dingyue"><span></span>订阅({{this.data.auditStatus}})</span>
          </div>
        </div>
      </div>
      <div class="buttonbox">
        <span class="iconfont">&#xe768;<span></span>播放</span>
        <span class="iconfont">&#xe604;<span></span>({{this.data.pubStatus}})</span>
        <span class="iconfont">&#xe669;<span></span>({{this.data.bdAuditStatus}})</span>
      ...
      <div class="pinglun">
        ...
        <div class="pingluntext">
			<div class="pinglunicon"><img :src="this.loginuser.avatarUrl" alt="" /></div>
		...
		<div class="newcomment">
				<div class="newcommentname">最新评论({{this.data.commentCount}})</div>
				<div class="newcommentbox">
					<div class="commenitem" v-for="(item) in this.comments.comments" :key="item.commentId" >
						<div class="commentIcon"><img :src="item.user.avatarUrl" alt=""></div>
						<div class="commentcontnt">
							<div><span class="nickname">{{item.user.nickname}}  :  </span><span>{{item.content}}</span></div>
							<div><span class="creartime">{{item.creatTime}}</span><span class="dian"><span class="iconfont ">&#xe604;</span><span>  | 回复</span></span></div>
						</div>
					</div>
				</div>
			</div>
	...
	methods:{
		...
		async getDtComment(id){
			...
			this.comments.comments.forEach(item => {
				...
				var M =creartime.getMonth() + 1;
				var d = creartime.getDate();
				var hh = (creartime.getHours() < 10 ? '0' + creartime.getHours() : creartime.getHours()) + ':';
				var mm = (creartime.getMinutes() < 10 ? '0' + creartime.getMinutes() : creartime.getMinutes()) + ':'
				item.creatTime=y+"年"+M+"月"+d+"   "+hh+"时"+mm+"分"
			});
		}
	},
	created() {
...
@font-face {
  ...
  src: url('//at.alicdn.com/t/font_2223549_l9r7k5bguie.eot?#iefix') format('embedded-opentype'),
